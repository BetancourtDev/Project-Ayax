...
class AjaxEndpoints
{
    ...
    public function savePaymentMethodSettings()
    {
        ...
        $settings = apply_filters_deprecated(
            ...
            FLUENTFORM_FRAMEWORK_UPGRADE,
            'fluentform_payment_method_settings_save_' . $method,
            'Use fluentform_payment_method_settings_save_' . $method . ' instead of fluentform_payment_method_settings_save_' . $method
        );
    ...
    public function saveCoupon()
    {
        ...
        if ($exist = (new CouponModel())->isCouponCodeAvailable($coupon['code'], $couponId)) {
            wp_send_json([
                'errors'  => [
                    'code' => [
                        'exist' => 'Same coupon code already exists'
                    ]
                ],
                'message' => 'Same coupon code already exists'
            ], 423);
        }
