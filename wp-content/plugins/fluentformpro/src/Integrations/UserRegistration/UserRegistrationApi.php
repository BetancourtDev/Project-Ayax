    ...
    public function registerUser($feed, $formData, $entry, $form, $integrationKey)
    {
        if (get_current_user_id()) {
            return $this->addLog(
                $feed['settings']['name'],
                'failed',
                "user registration skip because form submitted from login session.",
                $form->id,
                $entry->id,
                $integrationKey
            );
        ...
        if (!is_email($parsedValue['Email'])) return $this->addLog(
	        $feed['settings']['name'],
	        'failed',
	        "user registration skip because email is not valid.",
	        $form->id,
	        $entry->id,
	        $integrationKey
        );
        ...
        if (email_exists($parsedValue['Email'])) return $this->addLog(
	        $feed['settings']['name'],
	        'failed',
	        "user registration skip because email is already taken other.",
	        $form->id,
	        $entry->id,
	        $integrationKey
        );
        ...
        if (!empty($parsedValue['username'])) {
            ...
            if (is_array($userName)) {
	            return $this->addLog(
		            $feed['settings']['name'],
		            'failed',
		            "user registration skip because username is not valid data-type.",
		            $form->id,
		            $entry->id,
		            $integrationKey
	            );
            ...
            if ($userName && username_exists(sanitize_user($userName))) {
	            return $this->addLog(
		            $feed['settings']['name'],
		            'failed',
		            "user registration skip because username is already taken other.",
		            $form->id,
		            $entry->id,
		            $integrationKey
	            );
