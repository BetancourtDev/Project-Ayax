...
class DraftSubmissionsManager
{
    ...
    public function maybeLoadSavedProgress()
    {
        ...
        add_action('fluentform/before_form_render', function () use ($key) {
            wp_localize_script('form-save-progress', 'form_state_save_vars', [
                'source_url'          => home_url($_SERVER['REQUEST_URI']),
                'key'                 => $key,
                'nonce'               => wp_create_nonce(),
                'copy_button'         => sprintf("<img src='%s' >", fluentFormMix('public/img/copy.svg')),
                'copy_success_button' => sprintf("<img src='%s' >", fluentFormMix('public/img/check.svg')),
                'email_button'        => sprintf("<img src='%s' >", fluentFormMix('public/img/email.svg')),
            ]);
        });
    }
    ...
    private function deleteSavedStateDraft($form, $formData)
    {
        ...
        if ($draft) {
            ...
            do_action_deprecated(
                ...
                'Use fluentform_saved/progress_submission_deleted instead of fluentform_saved_progress_submission_deleted.'
            );
