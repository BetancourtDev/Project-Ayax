...
class PostFormHandler
{
    ...
    public function onFormSubmissionInserted($entryId, $formData, $form)
    {
        ...
        foreach ($feeds as $feed) {
            ...
            if (isset($feed->value['acf_mappings'])) {
                $feed->value['acf_mappings'] = AcfHelper::resolveAcfDateFieldFormat($feed->value['acf_mappings'], $form);
            }
        ...
        foreach ($formInputs as $field) {
            if ($field['element'] == 'featured_image') {
                ...
                if (!empty($formData[$fieldName][0])) {
                    ...
                    'update' == ArrayHelper::get($feed, 'post_form_type') &&
                    $postId = ArrayHelper::get($formData, '__ff_update_post_id')
                ) {
    ...
    protected function mapMetaFields($feed, $postData, $postType, $formData = [])
    {
        ...
        foreach ($metaInputs as $metaFieldMapping) {
            ...
            //maybe delete attachment's and update rest of the id
            if ($isFile && ($deletedIds = static::maybeRemoveMetaImages($metaFieldMapping['meta_value']))) {
                $metaFieldMapping['meta_value'] = join(',', array_diff($existingAttachmentIds, $deletedIds));
            } elseif ($isFile) {
                //make attachment's
